...
from gr_autopilot.launch import build_launch_plan, ranked_review_targets, render_markdown


def _read(book_id: int, title: str, author: str, rating: int,
          *, avg: float = 4.0, reviewed: bool = False) -> BookFact:
    return BookFact(
        book_id=book_id, title=title, author=author, my_rating=rating, avg_rating=avg,
        exclusive_shelf="read", date_read="2024/01/01",
        has_review=reviewed, review_text="x" if reviewed else "",
    )
